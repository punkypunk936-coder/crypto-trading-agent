...
try:
    ...
    TRADEXYZ_ASSET_METADATA = {}


_TRADEXYZ_CATEGORY_SEGMENTS = {
    "pre_ipo": "Pre-IPO",
    "mag7": "Megacap AI",
...
_TRADEXYZ_STRUCTURAL_LONG_SEGMENTS = {
    "AI Compute",
    "Optics",
    "Memory",
    "Interconnect",
...
def _default_tradexyz_assets_config() -> dict[str, dict[str, Any]]:
    ...
    for coin, meta in dict(TRADEXYZ_ASSET_METADATA or {}).items():
        ...
        assets[coin_upper] = {
            ...
            "categories": _normalize_asset_category_values((meta or {}).get("categories") or ["other_stocks"]),
...
def _normalize_tradexyz_assets(raw: Any) -> dict[str, dict[str, Any]]:
    assets: dict[str, dict[str, Any]] = {}
    for key, value in dict(raw or {}).items():
        coin = str(key or "").upper().strip()
        if not coin:
        ...
        categories = _normalize_asset_category_values(meta.get("categories") or meta.get("asset_categories") or [])
        assets[coin] = {
...
def _runtime_dashboard_config_defaults() -> dict:
    ...
    return {
        ...
        "tradexyz_assets": _default_tradexyz_assets_config(),
    }
...
def _merge_dashboard_config(config: Any) -> dict:
    ...
    merged["tradexyz_assets"] = _normalize_tradexyz_assets(defaults.get("tradexyz_assets") or {})
    merged["tradexyz_assets"].update(_normalize_tradexyz_assets(current.get("tradexyz_assets") or {}))
    return merged
...
def _tradexyz_segment_for_coin(coin: str, categories: list[str], instrument_type: str) -> str:
    coin_upper = str(coin or "").upper().strip()
    if coin_upper in _TRADEXYZ_SEGMENT_OVERRIDES:
        return _TRADEXYZ_SEGMENT_OVERRIDES[coin_upper]
    for category in categories or []:
        segment = _TRADEXYZ_CATEGORY_SEGMENTS.get(str(category or "").strip().lower())
        if segment:
            return segment
...
def _tradexyz_segment_key(segment: str) -> str:
    return "_".join(str(segment or "other").strip().lower().replace("/", " ").split()) or "other"


def _tradexyz_segment_rank(segment: str) -> int:
    try:
        return _TRADEXYZ_SEGMENT_ORDER.index(segment)
    except ValueError:
        return len(_TRADEXYZ_SEGMENT_ORDER) + 1


def _tradexyz_name_thesis(
    coin: str,
    display_name: str,
    segment: str,
    item: dict | None,
    ...
    override = _TRADEXYZ_NAME_THESES.get(str(coin or "").upper().strip())
    if override:
        return override
    return _clip_text(
...
def _tradexyz_htf_view(coin: str, segment: str, item: dict | None, sig: dict | None, pos: dict | None) -> tuple[str, str, bool, float]:
    item = dict(item or {})
    sig = dict(sig or {})
    pos = dict(pos or {})
    ...
    structural = segment in _TRADEXYZ_STRUCTURAL_LONG_SEGMENTS
    bearish = (
        direction == "SHORT"
...
def _tradexyz_assets_from_state(state: dict) -> dict[str, dict[str, Any]]:
    config = dict((state or {}).get("config") or {})
    assets = _normalize_tradexyz_assets(config.get("tradexyz_assets") or {})
    signals = dict((state or {}).get("signals") or {})
    configured = _normalize_coin_list(
        (config.get("coins") or [])
...
def build_xyz_section(state: dict, board: dict | None = None) -> dict:
    ...
    board_items = {
        str(item.get("coin") or "").upper(): dict(item or {})
        for item in list((board or {}).get("items") or [])
        if isinstance(item, dict) and str(item.get("coin") or "").strip()
    }
    signals = {
        str(coin or "").upper(): dict(sig or {})
        for coin, sig in dict(safe_state.get("signals") or {}).items()
    }
    positions = {
        str(pos.get("coin") or "").upper(): dict(pos or {})
        for pos in list(safe_state.get("positions") or [])
        if isinstance(pos, dict) and str(pos.get("coin") or "").strip()
    }
    assets = _tradexyz_assets_from_state(safe_state)
    items: list[dict[str, Any]] = []
    segment_counts: dict[str, dict[str, Any]] = {}

    for coin in sorted(assets.keys(), key=lambda value: (_tradexyz_segment_rank(_tradexyz_segment_for_coin(value, assets[value].get("categories") or [], assets[value].get("instrument_type") or "")), value)):
        meta = dict(assets.get(coin) or {})
        sig = dict(signals.get(coin) or {})
        item = dict(board_items.get(coin) or {})
        pos = dict(positions.get(coin) or {})
        instrument_type = str(meta.get("instrument_type") or _instrument_type_for_coin(coin, sig, config) or "equity").strip().lower()
        categories = _normalize_asset_category_values(meta.get("categories") or item.get("asset_categories") or [])
        if not categories:
            categories = _asset_categories_for_coin(coin, instrument_type, config)
        category_labels = [_asset_category_label(category, config) for category in categories]
        segment = _tradexyz_segment_for_coin(coin, categories, instrument_type)
        segment_key = _tradexyz_segment_key(segment)
        display_name = str(meta.get("display_name") or coin)
        htf_label, htf_reason, htf_hold, htf_score = _tradexyz_htf_view(coin, segment, item, sig, pos)
        current_action = str(item.get("label") or item.get("status") or sig.get("action") or "Watch").strip()
        name_thesis = _tradexyz_name_thesis(coin, display_name, segment, item, sig)
        segment_thesis = _TRADEXYZ_SEGMENT_THESES.get(segment) or _TRADEXYZ_SEGMENT_THESES["Other Stocks"]
        row = {
            "coin": coin,
            "name": display_name,
    ...
    items.sort(
        key=lambda row: (
            _tradexyz_segment_rank(str(row.get("segment") or "")),
    ...
    segments = sorted(
        ...
        key=lambda row: (_tradexyz_segment_rank(str(row.get("label") or "")), str(row.get("label") or "")),
    )
    return {
        "title": "xyz",
