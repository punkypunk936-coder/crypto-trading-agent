...
from tradexyz_profile import (
    build_xyz_section,
    default_tradexyz_assets_config,
    normalize_tradexyz_assets,
)
...
def _runtime_dashboard_config_defaults() -> dict:
    ...
    return {
        ...
        "asset_category_descriptions": _shared_asset_category_descriptions(),
        "tradexyz_assets": default_tradexyz_assets_config(),
    }
...
def _merge_dashboard_config(config: Any) -> dict:
    ...
    merged["asset_category_descriptions"] = dict(defaults.get("asset_category_descriptions") or {})
    merged["asset_category_descriptions"].update({
        str(key or "").strip().lower(): str(value or "")
        for key, value in dict(current.get("asset_category_descriptions") or {}).items()
        if str(key or "").strip()
    })
    merged["tradexyz_assets"] = normalize_tradexyz_assets(defaults.get("tradexyz_assets") or {})
    merged["tradexyz_assets"].update(normalize_tradexyz_assets(current.get("tradexyz_assets") or {}))
    return merged
...
def _asset_category_label(category: str, config: dict | None) -> str:
    return _shared_asset_category_label(category, config=config)
...
def build_dashboard_snapshot(
    ...
    *,
    ...
    global_context = global_market_context.build_global_market_context(
        shaped_state,
        market_map=normalized_market_map,
        asia_context=asia_context,
    ...
    return {
        ...
        "policy_health_report": dict(policy_health_report or {}),
