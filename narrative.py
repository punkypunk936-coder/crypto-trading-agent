...
def _coin_tags(coin: str) -> set[str]:
    ...
    if coin in {"NVDA", "INTC", "MU", "SNDK", "SKHX"}:
        tags.update({"EQUITY", "SEMIS_MEMORY", "RISK"})
    if coin in {"CRWV"}:
        tags.update({"EQUITY", "NEOCLOUD", "RISK"})
    if coin in {"EWY"}:
        tags.update({"INDEX", "ASIA", "MACRO", "RISK"})
    if coin in {"HIMS"}:
        tags.update({"EQUITY", "GROWTH", "RISK"})
