...
def test_analysis_only_watchlist_assets_are_never_promoted_for_execution() -> None:
    cfg = build_config()
    cfg.exchange.use_lighter = False
    cfg.exchange.use_hyperliquid = True
    cfg.trading.coins = ["BTC", "KR200"]
    cfg.trading.analysis_coins = ["BTC", "KR200", "EWY"]
    original_config = main_module.config
    original_supported = main_module.get_hyperliquid_supported_coins
    main_module.config = cfg
    try:
        main_module.get_hyperliquid_supported_coins = lambda **_kwargs: ["BTC", "KR200", "EWY"]
        active = main_module.enforce_trade_universe()
        assert active == ["BTC"]
        assert cfg.trading.coins == ["BTC"]
    finally:
        main_module.config = original_config
...
def test_analysis_only_asia_market_uses_reference_candles_when_venue_is_empty() -> None:
    original_post = market_data_module.requests.post
    original_supported = market_data_module.is_hyperliquid_supported
    original_resolve = market_data_module.resolve_hyperliquid_symbol
    original_dex = market_data_module.get_hyperliquid_market_dex
    original_yahoo = market_data_module._fetch_candles_yahoo
    original_cache = dict(market_data_module._cache)
    original_backoff = dict(market_data_module._candle_backoff_until)
    calls = []
    ...
    reference_df = pd.DataFrame([
        {"timestamp": pd.Timestamp("2026-08-11T00:00:00Z"), "open": 500.0, "high": 505.0, "low": 499.0, "close": 504.0, "volume": 10.0, "trades": 0},
        {"timestamp": pd.Timestamp("2026-08-11T01:00:00Z"), "open": 504.0, "high": 522.0, "low": 503.0, "close": 520.0, "volume": 12.0, "trades": 0},
    ])

    try:
        market_data_module._candle_backoff_until.clear()
        market_data_module.is_hyperliquid_supported = lambda _coin: True
        market_data_module.resolve_hyperliquid_symbol = lambda _coin: "xyz:KR200"
        market_data_module.get_hyperliquid_market_dex = lambda _coin: "xyz"
        market_data_module.requests.post = lambda *args, **kwargs: _Resp()

        def fake_yahoo(coin, interval, lookback, **kwargs):
            calls.append((coin, interval, lookback, kwargs))
            return reference_df

        market_data_module._fetch_candles_yahoo = fake_yahoo
        df = market_data_module.fetch_candles("KR200", "1h", 50)

        assert df is reference_df
        assert calls == [("KR200", "1h", 50, {"override_ticker": "^KS200", "cache_price": False})]
    finally:
        market_data_module.requests.post = original_post
        market_data_module.is_hyperliquid_supported = original_supported
        market_data_module.resolve_hyperliquid_symbol = original_resolve
        market_data_module.get_hyperliquid_market_dex = original_dex
        market_data_module._fetch_candles_yahoo = original_yahoo
        market_data_module._cache.clear()
        market_data_module._cache.update(original_cache)
        market_data_module._candle_backoff_until.clear()
        market_data_module._candle_backoff_until.update(original_backoff)


def test_asia_context_markets_are_analysis_only() -> None:
    trading_cfg = TradingConfig()
    for coin in trading_cfg.analysis_only_coins:
        assert coin in trading_cfg.analysis_coins
        assert coin not in trading_cfg.coins

    cfg = build_config()
    cfg.trading.coins = ["BTC", "KR200", "EWY"]
    exchange = DryRunExchange(starting_balance_usd=1000.0, supported_symbols=["BTC", "KR200", "EWY"])
    agent = TradingAgent(cfg, [exchange])

    assert agent._tradable_coins == ["BTC"]
    assert cfg.trading.coins == ["BTC"]
    result = agent._add_runtime_symbol(
        "KR200",
        executable=True,
        dynamic=True,
        spec={"instrument_type": "index", "categories": ["indices_macro"]},
    )
    assert result["execution_added"] is False
    assert "KR200" not in agent._tradable_coin_set
...
def test_stale_hyperliquid_candles_are_rejected_for_supported_market() -> None:
    ...
    original_resolve = market_data_module.resolve_hyperliquid_symbol
    original_dex = market_data_module.get_hyperliquid_market_dex
    original_spec = market_data_module.get_hyperliquid_market_spec
    original_cache = dict(market_data_module._cache)
    ...
    try:
        ...
        market_data_module.get_hyperliquid_market_dex = lambda _coin: "xyz"
        market_data_module.get_hyperliquid_market_spec = lambda _coin: {"instrument_type": "equity"}
        ...
        market_data_module.resolve_hyperliquid_symbol = original_resolve
        market_data_module.get_hyperliquid_market_dex = original_dex
        market_data_module.get_hyperliquid_market_spec = original_spec
        market_data_module._cache.clear()
        market_data_module._cache.update(original_cache)
...
def test_strong_korea_rally_is_risk_on_and_neutralizes_low_confidence_us_short() -> None:
    ...
    state = {
        "signals": {
            "SP500": {"price_action": {"bars": bars(7700.0)}},
            "KR200": {"price_action": {"bars": bars(950.0)}},
            "EWY": {"price_action": {"bars": bars(160.0)}},
        }
    }
    us_context = {
        "active": True,
        "regime": "RISK_OFF",
        "confidence": 0.50,
        "headline": "US structure is defensive but low-confidence.",
        "commentary": "US benchmarks are mixed.",
        "invalidation": "US indices reclaim together.",
        "benchmarks": [
            {"symbol": "SP500", "fresh": True, "direction": "SHORT"},
            {"symbol": "NDX", "fresh": True, "direction": "SHORT"},
        ],
        "breadth": {"net": -0.10},
        "execution_policy": {"preferred_direction": "SHORT"},
    }
    asia_context = {
        "regional_bias": "RISK_ON",
        "benchmarks": [
            {
                "symbol": "KR200", "region": "Korea", "fresh": True,
                "direction": "LONG", "move_pct": 5.01,
            },
            {
                "symbol": "EWY", "region": "Korea", "fresh": True,
                "direction": "LONG", "move_pct": 3.58,
            },
            {"symbol": "JP225", "region": "Japan", "fresh": True, "direction": "FLAT", "move_pct": 0.2},
        ],
        "us_readthrough": "Korea is constructive; US semis must confirm independently.",
    }
    ...
    korea = next(row for row in report["market_views"] if row["region"] == "Korea")
    assert korea["direction"] == "LONG"
    assert korea["status"] == "DIVERGING"
    assert korea["session_move_pct"] == 4.295
    assert "locally risk-on" in korea["summary"]
    assert report["cross_market"]["state"] == "DIVERGENT"
    assert report["cross_market"]["strong_asia_counter_signal"] is True
    assert report["cross_market"]["low_conviction_us_conflict"] is True
    assert report["cross_market"]["tactical_bias"] == "SELECTIVE"
    assert report["cross_market"]["display_bias"] == "SELECTIVE"
    assert "Korea is risk-on" in report["headline"]
    assert report["execution_policy"]["allow_tactical_short"] is False
...
def run_all() -> None:
    ...
    test_analysis_only_watchlist_assets_are_never_promoted_for_execution()
    print("PASS analysis-only watchlist isolation")
    ...
    test_analysis_only_asia_market_uses_reference_candles_when_venue_is_empty()
    print("PASS analysis-only Asia reference candle fallback")
    test_asia_context_markets_are_analysis_only()
    print("PASS Asia context execution isolation")
    ...
    test_strong_korea_rally_is_risk_on_and_neutralizes_low_confidence_us_short()
    print("PASS strong Korea rally neutralizes low-confidence US short")
